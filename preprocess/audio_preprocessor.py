'''
This code was largely repurposed from 
https://github.com/TimoBolkart/voca

I modified the code to make it compatible with pytorch
'''

import re
import copy
import resampy # resampling library to resample our audio files to 16Hz
import numpy as np
...
class AudioHandler:
    ...
    def convertAudioToInput(self, audio, fs, numcep, numcontext):
        # Get mfcc coefficients and keep only every second feature
        features = mfcc(audio, samplerate=fs, numcep=numcep)[::2]
        ...
        with tf.Session(graph=graph) as sess:
            for subj in audio.keys():
                for seq in audio[subj].keys():
                    ...
                    input_vector = self.convertAudioToInput(resampled_audio.astype('int16'), 16000, n_input, n_context)
